...
⚙️ **Mission Configurator**: Create, edit, and share custom missions without editing code. Add missions visually through a modal panel, export your mission list as a URL to share with others, or load missions from a shared URL — no server required. Each mission has its own ⚙ icon for quick in-place editing.
...
2. **Customize the intro** — the **Descripción principal (intro)** field at the top of the modal lets you edit the welcome text shown at the top of the page. Close the modal to save.
3. **Add a mission** — fill in the title and description fields, then click **Añadir misión**. Repeat for as many missions as you want.
4. **Edit a mission individually** — click the **⚙** icon next to any mission in the list to load it into the form and save changes.
5. **Edit or delete from the list** — in the mission list inside the modal, click **Editar** to reload a mission into the form, or **Eliminar** to remove it.
6. **Close the modal** — click the **×** button, press **Escape**, or click outside the modal panel.

### Share missions via URL

1. With your missions added in the configurator, click **Exportar a URL**.
2. The browser's address bar updates to include a `?data=...` parameter containing your mission list **and the intro text** (compressed and encoded).
3. Copy that URL and share it — anyone who opens it will have your missions and intro text pre-loaded.

### Load missions from a shared URL

Paste a URL with `?data=...` into the browser and open it. Mission Quest decodes the parameter and loads both the missions and the intro text automatically. This works entirely client-side — no server needed.
